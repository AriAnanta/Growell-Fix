...
export async function GET(request) {
  ...
  try {
    // Return latest survey for all balita owned by this orang_tua
    const [latestSurvey] = await pool.query(
    ...
    const mappedForm = {
      ...
      // Suplemen & Kehamilan
      rutinSuplemen: s.is_suplemen_kehamilan ? 'Rutin setiap hari' : 'Tidak pernah', // Simplified for boolean
      ...
      mpasiHewani: s.is_mpasi_hewani ? 'Ya, setiap hari' : 'Tidak pernah',
      ...
      tabletBesiAnak: s.is_tablet_besi_anak ? 'Ya' : 'Tidak',
      ...
      makananManis: s.is_konsumsi_manis_berlebih ? 'Ya, sering' : 'Tidak',
      bantuanGizi: s.is_pernah_pmt ? 'Ya' : 'Tidak',
      
      // Pola Hidup & Lingkungan
      jamTidur: s.jam_tidur_harian || '',
      aktivitasLuar: s.durasi_aktivitas_luar && s.durasi_aktivitas_luar > 3 ? 'Lebih dari 3 jam' : (s.durasi_aktivitas_luar > 1 ? '1-3 jam' : 'Kurang dari 1 jam'),
      tipeAnak: s.tingkat_aktivitas_anak || '',
      ibuBekerja: s.is_ibu_bekerja ? 'Ya' : 'Tidak',
      pengetahuanGizi: s.skor_pengetahuan_ibu > 80 ? 'Baik' : (s.skor_pengetahuan_ibu > 50 ? 'Cukup' : 'Kurang'),
      ...
      rutinPosyandu: s.frekuensi_posyandu_bulan > 8 ? 'Rutin setiap bulan' : (s.frekuensi_posyandu_bulan > 0 ? 'Jarang' : 'Tidak pernah'),
      ...
      frekPenyuluhan: s.frekuensi_kelas_ibu > 1 ? 'Setahun > sekali' : (s.frekuensi_kelas_ibu === 1 ? 'Setahun sekali' : 'Belum pernah'),
      pahamGizi: s.is_paham_makanan_sehat ? 'Paham' : 'Tidak paham',
